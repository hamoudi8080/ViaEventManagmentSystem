...
public class AcceptInvitationHandlerTest
{
    ...
    [Fact]
    public async Task
        AcceptInvitationHandler_WhenGuestAcceptsInvitation_ShouldAcceptInvitationAndBeAddedToParticipationsList()
    {
        // Arrange
        ...
        var acceptInvitationCommand = AcceptInvitationCommand
            .Create(viaEvent._eventId.Value.ToString(), guest.Id.Value.ToString()).Payload;
        ...
        Assert.True(viaEvent._Invitations.Any(x =>
            x._GuestId.Value == guest.Id.Value && x._InvitationStatus == InvitationStatus.Accepted));
