...

{
    ...
    {
        var queryInterfaceWithType = typeof(IQueryHandler<,>).MakeGenericType(query.GetType(), typeof(TAnswer));
        dynamic handler = serviceProvider.GetService(queryInterfaceWithType)!;

        if (handler == null) throw new ArgumentNullException(nameof(query));
