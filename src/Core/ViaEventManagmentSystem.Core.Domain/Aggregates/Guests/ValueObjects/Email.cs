...
public class Email : ValueObject
{
    ...
    public static Result<Email> Create(string input)
    {
        ...
        if (!IsValidEmailAddress(input))
            return Result<Email>.Failure(Error.BadRequest(ErrorMessage.TextFormatInvalid));
        ...
        if (parts.Length != 2 || !parts[1].Equals("via.dk", StringComparison.OrdinalIgnoreCase))
            return Result<Email>.Failure(Error.BadRequest(ErrorMessage.EmailMustEndWithViaDK));

        // Check the length and format of text1
        string text1 = parts[0];
        if (text1.Length < 3 || text1.Length > 6)
            return Result<Email>.Failure(Error.BadRequest(ErrorMessage.TextLengthOutOfRange));

        bool isValidEmail = (text1.Length == 3 || text1.Length == 4) && text1.All(char.IsLetter)
